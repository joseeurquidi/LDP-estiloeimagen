...
function sendRequest() {
	...
	jQuery('#contactWrapper').append('<div class="success"><h4>Mensaje enviado correctamente</h4><br><p>Gracias por escribirnos, nosotros nos pondremos en contacto contigo a la brevedad.</p></div>');
	}
}
...
function check_values() {
	...
	var errors=0;
	 ...
	 if($j('#contactform #name').val()=='') {
		...
		var hasClass=$j('#contactform #email').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #email').parent().append('<label for="contactname" generated="true" class="error">Por favor ingresa un correo válido</label>');	
		...
		var hasClass=$j('#contactform #subject').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Este campo es obligatorio</label>');	
		...
		var hasClass=$j('#contactform #body').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #body').parent().append('<label for="contactname" generated="true" class="error">El mensaje no debe estar vacío</label>');	
	...
	if(errors==0) {
			...
			document.getElementById("submit").value='Enviando el mensaje...';
			sendRequest();
	}
}
